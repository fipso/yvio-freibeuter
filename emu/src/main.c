/* main.c — M1 emulator entry: load app.elf, wire syscalls, run the scheduler. */
#include "cpu.h"
#include "elf.h"
#include "emu.h"
...
int main(int argc, char **argv)
{
    ...
    for (int i = 1; i < argc; i++) {
        if (!strcmp(argv[i], "--root")  && i + 1 < argc) root = argv[++i];
            ...
            if (!strcmp(argv[++i], "battle")) g_emu.scenario = 1;
        }
    ...
    char path[1024];
    snprintf(path, sizeof path, "%s/pirates/app.elf", root);
    printf("yvio-emu M1: loading %s (lang=%s)\n", path, g_emu.lang);
